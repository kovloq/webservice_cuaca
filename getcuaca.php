...
$query="SELECT * FROM cuaca JOIN kota ON kota.id=cuaca.kota_id where kota.name='".$_POST["kota"]."' AND tanggal LIKE '%".$tanggal."%'";
$result=mysql_query($query);
$data=mysql_fetch_array($result);
$query2="SELECT * FROM cuaca JOIN kota ON kota.id=cuaca.kota_id where kota.name='".$_POST["kota"]."' AND tanggal LIKE '%".$besok."%'";
...
if($_POST["propinsi"]=="Jabodetabek"){
	...
	$tanggal_besok=str_replace($arr_orig, $arr_rep, trim($data["tanggal"]));
	$arr=array(
		"name"=>$data["name"],
	 	"cuaca"=>$cuaca[0],
		"image"=>$image[0],
